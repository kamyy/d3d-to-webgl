...
namespace eve {
	P3N3::P3N3() : Shader(getName(), VtxMgr::DI_P3N3, g_vs20_main, g_ps20_main) {}

	void P3N3::setVSConstData(const Material* mat) const {
	...
	void P3N3::setPSConstData(const Material* mat) const {
		...
		setPSConstReg(2, 1, mat->getDiff());

		setPSConstReg(3, 1, s_omniLS->m_color * mat->getDiff());
		...
		setPSConstReg(5, 1, up * !s_frame->getWorldMatrix()); // world up in model space
	}
}
