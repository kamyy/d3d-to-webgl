...
export default class Shader {
  constructor(vertShaderURL, fragShaderURL) {
    ...
    const promise = Promise.all([createXHR(vertShaderURL), createXHR(fragShaderURL)])
  ...
  drawTriangleEdges(model, modelPiece) {
    ...
    if (currScene) {
      const camPosition = rawScenes[currScene.id].activeCamera.mapPos(g_origin, model) // map camera position into model space
      ...
      for (let i = 0; i < idxs.length; i += 3) {
        ...
        if (vx * nx + vy * ny + vz * nz > 0) {
          // back face cull using 1st vertex of triangle
