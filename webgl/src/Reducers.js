...
function onSceneLoad(scene) {
  const { id, cameras, omniDirLS, mapOfMaterials } = scene
...
function changeFieldOfView(state, fieldOfView) {
  return state.cameras.map((c, i) =>
    state.cameraIdx === i ? { fieldOfView: fieldOfView, aspectRatio: c.aspectRatio } : c
  )
}

function changeAspectRatio(state, aspectRatio) {
  return state.cameras.map((c, i) =>
    state.cameraIdx === i ? { fieldOfView: c.fieldOfView, aspectRatio: aspectRatio } : c
  )
}
