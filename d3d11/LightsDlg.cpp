...
namespace d3d11demo {
    ...
    float LightsDlg::s_attenCoeff0 = 1.0000f;
    float LightsDlg::s_attenCoeff1 = 0.0020f;
    float LightsDlg::s_attenCoeff2 = 0.0006f;

    const float kMinRGB = 0.0f;
    const float kMaxRGB = 3.0f;
    const float kMinCoeff0 = 1.0f;
    const float kMaxCoeff0 = 8.0f;
    const float kMinCoeff1 = 0.00f;
    const float kMaxCoeff1 = 0.10f;
    const float kMinCoeff2 = 0.00f;
    const float kMaxCoeff2 = 0.01f;
    ...
    void LightsDlg::OnHScroll(UINT code, UINT pos, CScrollBar* scrollBar) {
        ...
        float value = (float)slider->GetPos();
        char str[12];
        ...
        case IDC_UHG_SLIDER:
        case IDC_UHB_SLIDER:
            if (g_app.getAmbientLS()) {
                g_app.getAmbientLS()->m_lowerHemisphereColor = s_lowerHemisphereRGB;
                g_app.getAmbientLS()->m_upperHemisphereColor = s_upperHemisphereRGB;
            }
            break;
        case IDC_ODR_SLIDER:
        case IDC_ODG_SLIDER:
        ...
        case IDC_C1_SLIDER:
        case IDC_C2_SLIDER:
            if (g_app.getOmniDirLS()) {
                g_app.getOmniDirLS()->m_color = s_omniDirectionalRGB;
                g_app.getOmniDirLS()->m_coeff0 = s_attenCoeff0;
                g_app.getOmniDirLS()->m_coeff1 = s_attenCoeff1;
                g_app.getOmniDirLS()->m_coeff2 = s_attenCoeff2;
            }
            break;
        }
    }
